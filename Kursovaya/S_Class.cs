...
namespace Kursovaya
{
    public partial class S_Class : Form
    {
        public S_Class()
        {
            ...
            MySqlCommand command = new MySqlCommand("SELECT * FROM `s-class`", db.getConnection());
        ...
        private void buttonDoDelete_Click(object sender, EventArgs e)
        {
            ...
            if (table.Rows.Count > 0)
            {
                ...
                MySqlCommand command1 = new MySqlCommand("DELETE FROM `s-class` WHERE `s-class`.`id` = @Nid", db.getConnection());
